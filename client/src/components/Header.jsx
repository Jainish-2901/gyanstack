...
export default function Header() {
  ...
  const [isLegalOpen, setIsLegalOpen] = useState(false); // State for Legal Dropdown
    ...
    <nav className="navbar navbar-expand-lg border-0 fancy-header sticky-top">
      ...
      <div className="container-fluid">
        ...
        <div className={`navbar-collapse ${isNavOpen ? 'show mt-3 border-top pt-3' : 'collapse'}`} id="navbarNav">
          ...
          {/* User Profile Info (Mobile Sidebar Only) */}
          {user && (
          ...
          <ul className="navbar-nav me-auto mb-2 mb-lg-0 gap-lg-1">
            ...
            <li className="nav-item">
              <NavLink className={activeClass} to="/about" onClick={closeNav}>About</NavLink>
            </li>
            {/* --- DESKTOP LEGAL DROPDOWN --- */}
            <li className="nav-item dropdown d-none d-lg-block position-relative">
              <button
                className="nav-link border-0 bg-transparent d-flex align-items-center gap-1"
                onClick={() => setIsLegalOpen(!isLegalOpen)}
              >
                Legal <i className={`bi bi-chevron-${isLegalOpen ? 'up' : 'down'} small opacity-50`}></i>
              </button>
              {isLegalOpen && (
                <ul className="dropdown-menu shadow-lg border-0 glass-panel show position-absolute mt-2">
                  <li><Link className="dropdown-item py-2 fw-bold" to="/privacy" onClick={() => setIsLegalOpen(false)}>Privacy Policy</Link></li>
                  <li><Link className="dropdown-item py-2 fw-bold" to="/terms" onClick={() => setIsLegalOpen(false)}>Terms of Service</Link></li>
                </ul>
              )}
            </li>
            ...
            <li className="nav-item d-lg-none">
              <Link to="/privacy" className="nav-link text-muted" onClick={closeNav}>
                <i className="bi bi-shield-lock-fill me-2"></i> Privacy Policy
              </Link>
            </li>
            <li className="nav-item d-lg-none">
              <Link to="/terms" className="nav-link text-muted" onClick={closeNav}>
                <i className="bi bi-file-earmark-text-fill me-2"></i> Terms of Service
              </Link>
            </li>
            <li className="nav-item d-lg-none">
              <Link to="/about" className="nav-link text-muted" onClick={closeNav}>
            ...
            {/* Mobile Login/Signup */}
            {!user && isNavOpen && (
              <li className="nav-item d-lg-none border-top mt-2 pt-3">
                <div className='d-flex flex-column gap-2'>
                  <Link className="btn btn-outline-primary fw-bold w-100" to="/login" onClick={closeNav}>Login</Link>
                  <Link className="btn btn-primary fw-bold w-100" to="/signup" onClick={closeNav}>Sign Up</Link>
                </div>
              </li>
            )}
          </ul>

          {/* --- DESKTOP User Actions (Login/Signup OR Profile Dropdown) --- */}
