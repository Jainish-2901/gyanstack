...
const DetailPreview = ({ item }) => {
  ...
  if (item.fileResourceType === 'video' || item.type.includes('video')) {
    return (
      <div>
        <div className="ratio ratio-16x9 shadow-lg rounded bg-black">
          {/* Removed autoPlay to prevent auto-download/auto-streaming bandwidth usage */}
          <video controls>
  ...
  // 6. Office Documents (Word, PPT, Excel) - Preview using Google Docs Viewer
  const isOfficeDoc = item.type.includes('officedocument') || 
                      item.type.includes('msword') || 
                      item.type.includes('powerpoint') || 
                      item.type.includes('excel') ||
                      item.url.match(/\.(doc|docx|ppt|pptx|xls|xlsx)$/i);

  if (isOfficeDoc) {
    const googleViewerUrl = `https://docs.google.com/gview?url=${encodeURIComponent(item.url)}&embedded=true`;
    return (
      <div>
        <div className="ratio ratio-4x3 shadow-lg rounded border">
          <iframe src={googleViewerUrl} title={item.title}></iframe>
        </div>
        <div className="mt-2 text-center text-muted">
          <small><i className="bi bi-info-circle me-1"></i> Preview powered by Google Docs Viewer</small>
        </div>
      </div>
    );
  }
  
  // 7. 'Raw' File Type (ZIP, etc.)
  if (item.fileResourceType === 'raw') {
    ...
    return (
      <div className="text-center p-5 bg-light rounded shadow-sm border">
        ...
        <h3 className='mt-3'>File Content</h3>
        <p className="lead">This file type ({item.type.split('/')[1] || 'binary'}) cannot be previewed directly.</p>
        <p className='text-muted'>Please use the Download button below to view the contents.</p>
      </div>
    );
  }
      ...
      <div className="row justify-content-center">
        <div className="col-lg-10">
          ...
          <div className="d-flex gap-3 mb-5 flex-wrap">
            <button 
              className={`btn btn-lg rounded-pill px-4 shadow-sm ${isLiked ? 'btn-danger' : 'btn-outline-danger'}`}
              onClick={handleLike}
              disabled={!user}
            >
            ...
            <button 
              className={`btn btn-lg rounded-pill px-4 shadow-sm ${isSaved ? 'btn-success' : 'btn-outline-success'}`}
              onClick={handleSave}
              disabled={!user}
            >
            ...
            {/* Show Download Button for everything that has a URL and is not a simple external link or note */}
            {item.url && item.type !== 'link' && item.type !== 'note' && (
              <button 
                onClick={handleDownload}
                disabled={!user}
                className="btn btn-lg btn-info rounded-pill px-4"
              >
